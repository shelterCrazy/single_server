...
var matchPool = [];
matchPool["jojo"] = [];
matchPool["ConnectorOMEGA"] = [];

var gameIdList = ["jojo", "ConnectorOMEGA"];
    ...
    index.on("connection", function (socket) {
        ...
        socket.on('roomMsg', function (data) {
            if (data.room != null && data.room != "") {
                //在现有的房间中查找房间号码一致的房间，得到序号一致的I
                for (var i = 0; i < roomPool.length; i++) {
                    if (data.room == roomPool[i].room){
                        break;
                    }
                }
                if(i == roomPool.length){
                    return;
                }
                if(roomPool[i].gameId == "jojo"){
        ...
        socket.on('match', function (data) {
            ...
            for(var k in gameIdList){
                ...
                while (matchPool[gameName][data.playerNum].length >= data.playerNum) {
                    ...
                    for (var i = 0; i < num; i++) {
                        roomData.socketPool.push(matchPool[gameName][num][i].socket),
                    ...
                    matchPool[gameName][roomData.playerNum].splice(0, data.playerNum);
                }
            }
        });
        //取消匹配的事件
                ...
                for(var k in gameIdList){
                    ...
                    for (var j = 0; j < matchPool[gameName][i].length; j++) {
                        ...
                        if (matchPool[gameName][i][j].socket.id == socket.id) {
                            matchPool[gameName][i].splice(j, 1);
                            break;
                        }
                    }
                }
